...
export function createLeadgenRouterAgent(prefilledOpeningText?: string) {
  const injectedOpening = prefilledOpeningText
    ? `\n\n# OPENING TỪ API (ƯU TIÊN CAO NHẤT)\n` +
      `Ở lượt đầu tiên, nói đúng nguyên văn câu sau và kết thúc |CHAT:\n` +
      `"${prefilledOpeningText} |CHAT"`
    : '';

  return new RealtimeAgent({
    name: 'leadgenTNDS',
    instructions: `${leadgenRouterInstructions}${injectedOpening}`,
