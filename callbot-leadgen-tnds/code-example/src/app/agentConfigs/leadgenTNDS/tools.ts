...
export function calculateTndsFee(args: {
  ...
  if (vehicleType === 'truck') {
    ...
    return {
      ...
      periodSavings:
        typeof discountedTotal === 'number' ? (band.price.total - discountedTotal) * years : undefined,
    };
  }
    ...
    if (!band) {
      if ((seats || 0) > 25) {
        ...
        return {
          ...
          periodSavings:
            typeof computed.discountedTotal === 'number'
              ? (computed.total - computed.discountedTotal) * years
              : undefined,
        };
      }
      return { ok: false, error: 'Khong tim thay bang phi phu hop.' };
...
export const calcTndsFeeTool = tool({
  ...
  parameters: {
    ...
    properties: {
      ...
      durationYears: { type: 'number', description: 'So nam bao gia, mac dinh 1 nam' },
    },
    required: ['vehicleType'],
    additionalProperties: false,
  },
  execute: async (args: any) => {
    return calculateTndsFee(args as {
      vehicleType: 'car' | 'pickup' | 'truck';
      seats?: number;
      isBusiness?: boolean;
      weightTons?: number;
