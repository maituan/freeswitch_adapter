...
import {
  createLeadgenFallbackStillValidGuardrail,
  createLeadgenFaqToolRequiredGuardrail,
  createLeadgenOpeningGuardrail,
  createModerationGuardrail,
} from "@/app/agentConfigs/guardrails";
import { createLeadgenVer02EvaluateGuardrail } from "@/app/agentConfigs/leadgenTNDSVer02/internal/outputGuardrails";
...
import {
  createLeadgenRouterAgent as createLeadgenVer02RouterAgent,
  leadgenTndsVer02Scenario,
  setLeadgenVer02RuntimeContext,
} from "@/app/agentConfigs/leadgenTNDSVer02";
...
function App() {
  ...
  const [leadgenAgentName, setLeadgenAgentName] = useState<string>(
    () => searchParams.get('agentName') || 'Thảo',
  );
  const [leadgenPlate, setLeadgenPlate] = useState<string>(
    () => searchParams.get('leadPlate') || '',
  );
  const [leadgenVehicleType, setLeadgenVehicleType] = useState<string>(
    () => searchParams.get('vehicleType') || '',
  );
  const [leadgenNumSeats, setLeadgenNumSeats] = useState<string>(
    () => searchParams.get('numSeats') || '',
  );
  const [leadgenIsBusiness, setLeadgenIsBusiness] = useState<string>(
    () => searchParams.get('isBusiness') || '',
  );
  const [leadgenWeightTons, setLeadgenWeightTons] = useState<string>(
    () => searchParams.get('weightTons') || '',
  );
  ...
  useEffect(() => {
    if (agentSetKey !== 'leadgenTNDSVer02') return;
  ...
  useEffect(() => {
    ...
    if (
      finalAgentConfig === 'leadgenTNDS' ||
      finalAgentConfig === 'leadgenTNDSVer02' ||
      finalAgentConfig === 'carebotAuto365'
    ) {
    ...
    if (sdkScenarioMap[agentSetKey]) {
      ...
      try {
        ...
        if (agentSetKey === 'leadgenTNDS') {
          // Keep opening deterministic from getLeadContext tool only, so the first turn
          // is generated from a single source of truth and won't be duplicated.
          reorderedAgents[0] = createLeadgenRouterAgent();
        }

        if (agentSetKey === 'leadgenTNDSVer02') {
          reorderedAgents[0] = createLeadgenVer02RouterAgent();
        }
        ...
        if (agentSetKey === 'leadgenTNDS') {
          ...
          guardrails.push(createLeadgenFallbackStillValidGuardrail());
        }
        if (agentSetKey === 'leadgenTNDSVer02') {
          guardrails.push(createLeadgenVer02EvaluateGuardrail());
        }
  ...
  return (
    <div className="text-base flex flex-col h-screen bg-gray-100 text-gray-800 relative">
      ...
      {(agentSetKey === 'leadgenTNDS' || agentSetKey === 'leadgenTNDSVer02') && (
