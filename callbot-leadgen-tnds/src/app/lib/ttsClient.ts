...
function normalizeText(text: string): string {
  ...
  return expandAbbreviations(normalizeCurrencyAmounts(normalizeLicensePlates(text)))
    ...
    // Pricing special-case: avoid reading "đồng hoặc năm"
    .replace(/đồng\s*\/\s*năm/giu, 'đồng một năm')
    .replace(/VNĐ\s*\/\s*năm/giu, 'đồng một năm')
    .replace(/VND\s*\/\s*năm/giu, 'đồng một năm')
