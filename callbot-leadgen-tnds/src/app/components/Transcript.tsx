...
function Transcript({
  ...
  const isFastPricingBuffer = (text: string) => {
    const normalized = String(text || '')
      ...
      .replace(/\s+/g, ' ')
      .trim();
    return normalized === 'anh cho em mot chut de em kiem tra gia nhe';
  };
  ...
  useEffect(() => {
    if (asrError) {
      console.warn("[Transcript] ASR Error:", asrError);
    }
  }, [asrError]);
  ...
  }, [chatOnly, partialTranscript, isASRRecording]);
  ...
  useEffect(() => {
    ...
    if (hasTextChanged) {
      ...
      pendingAutoPlayRef.current = {
        ...
        }, isFastPricingBuffer(latestText) ? 80 : 650),
      };
    }
  }, [transcriptItems, autoPlayTTS, enableTTS, isPlaying, isLoading, speak, ttsMode, ttsVoiceId, callSession, agentSetKey]);
    ...
    <div className="flex flex-col flex-1 bg-white min-h-0 rounded-xl">
      ...
      <div className="p-4 flex items-center gap-x-2 flex-shrink-0 border-t border-gray-200">
        ...
        <input
          ...
          onKeyDown={(e) => {
            // On macOS/IME, Enter during composition can duplicate trailing text.
          ...
          placeholder={chatOnly ? "Chat-only mode: nhập tin nhắn..." : "Type a message..."}
        />
        <button
