...
export function useHandleSessionHistory() {
  ...
  const extractMessageText = (content: any[] = []): string => {
    ...
    return content
      .map((c) => {
        ...
        if (c.type === "text") return c.text ?? ""; // Text-only response from assistant
        if (c.type === "output_text") return c.text ?? ""; // Responses API / realtime output
        if (c.type === "output_text_delta") return c.delta ?? "";
  ...
  const hasDeltaContent = (content: any[] = []): boolean => {
    if (!Array.isArray(content)) return false;
    return content.some((c: any) => c?.type === "output_text_delta");
  };
  ...
  function handleAgentToolStart(details: any, _agent: any, functionCall: any) {
    ...
    // Also send to server terminal for debugging.
    try {
      fetch('/api/debug', {
        method: 'POST',
    ...
    // For pricing in leadgen ver02, inject a local waiting line immediately at tool start
    // so the customer hears it before the pricing tool returns.
    if (
      function_name === 'calcTndsFee' &&
      String(_agent?.name ?? '') === 'leadgenSalesAgent'
    ) {
      const localItemId = `local-calc-buffer-${Date.now()}`;
      addTranscriptMessage(localItemId, 'assistant', 'Anh chờ em một chút để em kiểm tra giá nhé.');
      updateTranscriptItem(localItemId, { status: 'DONE' });
    }
  }
    ...
    try {
      fetch('/api/debug', {
        ...
        body: JSON.stringify({
          source: 'sdk',
          event: 'tool_end',
          data: { name: lastFunctionCall?.name, result: maybeParseJson(result) },
        }),
      }).catch(() => {});
    } catch {}
  }
  ...
  function handleHistoryUpdated(items: any[]) {
    ...
    items.forEach((item: any) => {
      ...
      const isDelta = hasDeltaContent(content);
